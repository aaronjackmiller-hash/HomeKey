...
RoommateListingSchema.pre('validate', function localizeAddressBeforeValidate(next) {
    ...
    // Stash coordinates before buildLocalizedAddress replaces the address object —
    // that function doesn't pass lat/lng through, so they'd be silently dropped
    // and never saved to MongoDB.
    const savedLat = this.address.lat;
    const savedLng = this.address.lng;
    this.address = buildLocalizedAddress(this.address);
    if (typeof savedLat === 'number' && Number.isFinite(savedLat)) this.address.lat = savedLat;
    if (typeof savedLng === 'number' && Number.isFinite(savedLng)) this.address.lng = savedLng;
    return next();
});
